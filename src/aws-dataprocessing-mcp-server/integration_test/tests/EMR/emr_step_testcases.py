...
def emr_step_test_cases(s3_bucket, aws_clients) -> List[MCPTestCase]:
    ...
    return [
        ...
        MCPTestCase(
            ...
            validators=[ContainsTextValidator('Successfully described step')],
            clean_ups=[],
        ),
        MCPTestCase(
            test_name='describe_emr_step_missing_ids',
            tool_name='manage_aws_emr_ec2_steps',
            input_params={'operation': 'describe-step'},
            dependencies=[],
            validators=[ContainsTextValidator('Field required')],
            clean_ups=[],
        ),
        MCPTestCase(
            ...
            validators=[
                ContainsTextValidator(
                    'Successfully listed steps for EMR cluster', expected_count=1
                )
            ],
            clean_ups=[],
        ),
        MCPTestCase(
            ...
            tool_name='manage_aws_emr_ec2_steps',
            input_params={'operation': 'list-steps', 'max_results': 10},
            dependencies=[],
            validators=[ContainsTextValidator('Field required')],
            clean_ups=[],
        ),
        MCPTestCase(
            ...
            input_params={
                ...
                'cluster_id': '{{create_emr_cluster_basic.result.content[0].text.cluster_id}}',  # Use the cluster_id from the dependency's response
                'step_ids': [
                    '{{add_step_to_emr_cluster.result.content[0].text.step_ids[0]}}'
                ],  # Use the step_id from the dependency's response
            },
            dependencies=['add_step_to_emr_cluster'],
            validators=[ContainsTextValidator('Successfully initiated cancellation for 1 steps')],
            clean_ups=[],
        ),
        MCPTestCase(
            test_name='cancel_emr_step_missing_ids',
            tool_name='manage_aws_emr_ec2_steps',
            input_params={'operation': 'cancel-steps'},
            dependencies=[],
            validators=[ContainsTextValidator('Field required')],
            clean_ups=[],
        ),
