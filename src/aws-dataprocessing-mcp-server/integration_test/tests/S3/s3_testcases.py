...
def s3_test_cases(s3_bucket, aws_clients) -> List[MCPTestCase]:
    ...
    return [
        MCPTestCase(
            ...
            input_params={},
            dependencies=[],
            validators=[ContainsTextValidator('Looking for S3 buckets')],
            clean_ups=[],
        ),
        MCPTestCase(
            test_name='list_s3_buckets_with_region',
            tool_name='list_s3_buckets',
            input_params={'region': os.environ.get('AWS_REGION', 'us-west-2')},
            dependencies=[],
            validators=[ContainsTextValidator('Looking for S3 buckets', bucket_count=0)],
            clean_ups=[],
        ),
        MCPTestCase(
            test_name='list_s3_buckets_with_invalid_region',
            tool_name='list_s3_buckets',
            input_params={'region': 'invalid-region'},
            dependencies=[],
            validators=[
                ContainsTextValidator(
                    "Error listing S3 buckets: An error occurred (InvalidRegion) when calling the ListBuckets operation: The region 'invalid-region' is invalid"
                )
            ],
            clean_ups=[],
        ),
        ...
        MCPTestCase(
            ...
            input_params={'bucket_name': None},
            dependencies=[],
            validators=[ContainsTextValidator('Analyzing bucket')],
            clean_ups=[],
        ),
        MCPTestCase(
            test_name='analyze_s3_usage_with_bucket',
            tool_name='analyze_s3_usage_for_data_processing',
            input_params={'bucket_name': f'{s3_bucket}'},
