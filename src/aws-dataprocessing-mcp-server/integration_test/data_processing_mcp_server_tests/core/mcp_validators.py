...
import json
import re
from data_processing_mcp_server_tests.models.validators import (
    BotoValidator,
    TextValidator,
    ValidationResult,
)
...
from data_processing_mcp_server_tests.utils.utils import (
    OPERATION_ARN_MAP,
...
class ContainsTextValidator(TextValidator):
    """Validator that checks if response contains expected text."""
    ...
    def validate(self, actual_response: Dict[str, any]) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                False, f"Expected string '{self.expected_string}' not found, and not valid JSON"
            )

        embedded_text = parsed_json.get('content', [{}])[0].get('text', '')
        if self.expected_string not in embedded_text:
            return ValidationResult(
                False, f"Expected string '{self.expected_string}' not found in response"
            )
        ...
        if self.expected_count is not None:
            ...
            if actual_count != self.expected_count:
                return ValidationResult(
                    False, f'Count mismatch: expected {self.expected_count}, got {actual_count}'
                )
        if self.bucket_count is not None:
            ...
            if actual_bucket_count != self.bucket_count:
                return ValidationResult(
                    False,
                    f'Bucket count mismatch: expected {self.bucket_count}, got {actual_bucket_count}',
                )
...
class AWSBotoValidator(BotoValidator):
    ...
    def _construct_arn(self, tool_params: Dict[str, Any]) -> Optional[str]:
        ...
        for key in param_key.split('/'):
            ...
            if val is None:
                raise ValueError(
                    f"Missing resource identifier '{key}' for operation '{self.operation}'"
                )
        ...
        if not resource_id:
            raise ValueError(
                f"Missing resource identifier '{param_key}' for operation '{self.operation}'"
            )

        return f'arn:aws:{service_name}:{region}:{account_id}:{resource_type}/{resource_id}'

    def _validate_expected_keys_by_operation(
        self, operation: str, tool_params: Dict[str, Any], response: Dict[str, Any]
    ) -> List[str]:
        mismatches = []

        for key_path in self.expected_keys:
            input_key_prefix, response_key_prefix = OPERATION_PREFIX_MAP.get(
                operation, (None, None)
            )
            ...
            if input_key_prefix is not None:
                sub_obj = (
                    tool_params
                    if input_key_prefix == ''
                    else tool_params.get(input_key_prefix, {})
                )
            ...
            if response_key_prefix is not None:
                actual_value = self._get_nested_value(
                    response.get(response_key_prefix, {}), key_path
                )
            else:
            ...
            if expected_value != actual_value:
                if isinstance(actual_value, list):
                    if expected_value not in actual_value:
                        mismatches.append(
                            f"Expected '{expected_value}' in list at '{key_path}', but got {actual_value}"
                        )
                else:
                    mismatches.append(
                        f"Mismatch for '{key_path}': expected '{expected_value}', got '{actual_value}'"
                    )

        return mismatches

    def validate(
        self, tool_params: Dict[str, Any], response_map: Optional[Dict[str, Any]] = None
    ) -> ValidationResult:
        ...
        Args:
            ...
            response_map: Optional map of responses from previous test executions.
        ...
        try:
            ...
            if self.injectable_params:
                ...
                for k, v in self.injectable_params.items():
                    ...
                    if match:
                        ...
                        if not dep_response:
                            return ValidationResult(
                                False, f'Missing response for dependency: {dep_name}'
                            )
                        try:
            ...
            if not method:
                return ValidationResult(
                    False, f"Operation '{self.operation}' not found in AWS client"
                )
            ...
            if self.validate_absence:
                ...
                if (
                    self.operation == 'get_trigger'
                    and response.get('Trigger', {}).get('State') == 'DELETING'
                ):
                    return ValidationResult(
                        True, 'Trigger is in DELETING state, considered as deleted'
                    )
                return ValidationResult(
                    False, f'Expected resource to NOT exist, but it does: {response}'
                )

            # Validate expected keys
            mismatches = self._validate_expected_keys_by_operation(
                self.operation, tool_params, response
            )

            # Check MCP tags
            if self.operation in SKIP_TAG_CHECK_OPERATIONS:
                logger.info(
                    f'[Validator Info] Skipping tag validation for operation: {self.operation}'
                )
            else:
                tags_dict = {}
                    ...
                    try:
                        ...
                        if hasattr(self.boto_client, 'list_tags_for_resource'):
                            ...
                            tags_dict = (
                                ...
                                else {tag['Key']: tag['Value'] for tag in tag_list}
                            )
                        else:
                            logger.warning(
                                f'[Validator Warning] No tag API available for {self.operation}'
                            )

                    except Exception as e:
                        logger.warning(
                            f'[Validator Warning] Failed to get tags for {self.operation}: {e}'
                        )
                ...
                if tags_dict.get('ManagedBy') != 'DataprocessingMcpServer':
                    mismatches.append(
                        f"ManagedBy should be 'DataprocessingMcpServer', got '{tags_dict.get('ManagedBy')}'"
                    )
            ...
            return ValidationResult(
                True, f"Validation successful for operation '{self.operation}'"
            )

        except botocore.exceptions.ClientError as e:
            error_code = e.response['Error'].get('Code', '')
            if self.validate_absence and error_code in [
            ...
            return ValidationResult(
                False,
                f'ClientError during validation: {error_code} - {e.response["Error"].get("Message", str(e))}',
            )
