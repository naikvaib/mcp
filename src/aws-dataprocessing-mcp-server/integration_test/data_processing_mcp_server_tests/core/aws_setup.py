...
def create_s3_bucket_if_not_exists():
    ...
    try:
        ...
        if error_code == 404:
            ...
            raise RuntimeError(
                f'[Setup Error] Unexpected error when checking/creating bucket: {e}'
            )
    return bucket_name
    ...
    for page in paginator.paginate():
        for role in page['Roles']:
            if role['RoleName'] == role_name:
                ...
                try:
                    role_policies = iam_boto_client.list_attached_role_policies(
                        RoleName=role['RoleName']
                    )
                    required_policies = ['AWSGlueServiceRole', 'service-role/AWSGlueServiceRole']
                    has_required = any(
                        p['PolicyName'] in required_policies
                        for p in role_policies['AttachedPolicies']
                    )
                    if not has_required:
                        logger.warning(
                            f'Warning: Role {role["RoleName"]} may not have required Glue permissions'
                        )
...
def create_non_mcp_job(job_name='non-mcp-test-job'):
    ...
    This job doesn't have MCP managed tags.
    """
    try:
        ...
        create_s3_bucket_if_not_exists()
        upload_script(
            script_name='non-mcp-script.py',
            s3_bucket=get_standard_bucket_name(),
            prefix='glue_job_script/',
        )
