    ...
    def clean_up(self, _: Dict[str, Any], actual_response: Dict[str, Any]):
        ...
        if self.resource_field and self.target_param_key:
            try:
                ...
                logger.info(f"[Cleanup] Successfully parsed resource ID: {resource_id} to delete params")
            ...
            if self.delete_api == "delete_session":
                ...
                if session_id:
                    for _ in range(60):  # wait max 60s
                        ...
                        logger.warning(f"[Cleanup Warning] Timeout waiting for session {session_id} to be deletable")
        ...
        if self.delete_api == "batch_stop_job_run":
            ...
            if job_name and job_run_ids:
                ...
                logger.info("Sleeping for 30 seconds to allow job run state transitions to complete...")
