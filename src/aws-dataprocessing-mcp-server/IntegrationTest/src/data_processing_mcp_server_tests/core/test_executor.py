...
class TestExecutor:
    """Test execution engine that manages test cases and their validation.

    This class provides functionality to run tests with proper setup/cleanup,
    validate test results, and generate reports. It coordinates test execution
    and manages the lifecycle of test cases.
    """

    def __init__(self, mcp_client: MCPClient):
        """Initialize a new test executor.

        Args:
            mcp_client: The MCP client to use for making tool calls
        """
    ...
    def run_test(
        ...
        try:
            ...
            # Execute MCP tool
            response = self.mcp_client.call_tool(test_case.tool_name, test_case.input_params)
    ...
    def run_tests_only(self, report_dir: str = 'test_reports') -> List[TestResult]:
        ...
        for i, result in enumerate(results, 1):
            test_status = (
                ...
                if result.success
                else f'{Fore.RED}FAIL: {result.error}{Style.RESET_ALL}'
            )
    ...
    def run_all_tests(
        self, report_formats: List[str] = None, report_dir: str = 'test_reports'
    ) -> Dict[str, Any]:
        ...
        for name, success, error in cleanup_results:
            status = (
                ...
                if success
                else f'{Fore.RED}FAILED: {error}{Style.RESET_ALL}'
            )
