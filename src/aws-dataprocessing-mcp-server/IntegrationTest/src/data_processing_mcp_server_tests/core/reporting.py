...
class ReportGenerator:
    ...
    def generate_report(
        self, results: List[TestResult], formats: List[str] = None
    ) -> Dict[str, str]:
    ...
    def _generate_markdown(self, results: List[TestResult]) -> str:
        ...
        lines = ['# Test Execution Report', f'Generated: {timestamp}', '', '## Summary', '']
            ...
            if result.validation_results:
                ...
                for j, validation in enumerate(result.validation_results, 1):
                    ...
                    if validation.details:
                        lines.append('     ```json')
                        lines.append(
                            f'     {json.dumps(validation.details, indent=2, cls=CustomEncoder)}'
                        )
                        lines.append('     ```')
    ...
    def _generate_json(self, results: List[TestResult]) -> str:
        ...
        for result in results:
            ...
            if result.validation_results:
                test_data['validations'] = [
                    {'success': v.success, 'message': v.message, 'details': v.details}
        ...
        if failed_results:
            ...
            report_data['error_summary'] = {
                error_type: [
                    {'test_name': case.test_case.test_name, 'error': case.error} for case in cases
                ]
                for error_type, cases in error_groups.items()
            }
    ...
    def _generate_html(self, results: List[TestResult]) -> str:
        ...
        for i, result in enumerate(results, 1):
            ...
            html.extend(
                [
                    ...
                    '',
                    '            <div><strong>Input Parameters:</strong></div>',
            ...
            # Validation Results
            if result.validation_results:
                html.append('            <div><strong>Validations:</strong></div>')
                ...
                for j, validation in enumerate(result.validation_results, 1):
                    ...
                    html.append(
                        f"            <div class='validation {status_class}'>{j}. {status_icon} {validation.message}</div>"
                    )

                    if validation.details:
                        html.append(
                            f'            <pre>{json.dumps(validation.details, indent=2, cls=CustomEncoder)}</pre>'
                        )
            ...
            if result.error:
                ...
                if fix:
                    html.extend(
                        [
                            '            <div><strong>Fix Recommendation:</strong></div>',
                            f'            <div>{fix}</div>',
                        ]
                    )
        ...
        if failed > 0:
            html.extend(
                ["        <div class='error-summary'>", '            <h2>Error Summary</h2>']
            )
            ...
            for error_type, cases in error_groups.items():
                ...
                for case in cases:
                    html.append(
                        f'                <li><strong>{case.test_case.test_name}</strong>: {case.error}</li>'
                    )
