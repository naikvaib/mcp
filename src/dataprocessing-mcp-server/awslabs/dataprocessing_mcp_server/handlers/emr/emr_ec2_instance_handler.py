...
class EMREc2InstanceHandler:
    ...
    def __init__(
        ...
        self.mcp.tool(name="manage_aws_emr_ec2_instances")(
            self.manage_aws_emr_ec2_instances
        )
    ...
    async def manage_aws_emr_ec2_instances(
        ...
        try:
            ...
            if operation == "add-instance-fleet":
                ...
                try:
                    existing_tags_response = self.emr_client.describe_cluster(ClusterId=str(cluster_id))
                    existing_tags = {tag["Key"]: tag["Value"] for tag in existing_tags_response.get("Cluster", {}).get("Tags", [])}
                    
                    # Check if required MCP tags are present
                    if MCP_MANAGED_TAG_KEY not in existing_tags or existing_tags.get(MCP_MANAGED_TAG_KEY) != MCP_MANAGED_TAG_VALUE:
                    ...
                    log_with_request_id(ctx, LogLevel.INFO, f"Resource is MCP managed, proceeding with instance fleet modification")
                ...
                if cluster_id:
                    try:
                        ...
                        # Resource is MCP managed, proceed with modification
                        log_with_request_id(ctx, LogLevel.INFO, f"Resource is MCP managed, proceeding with instance group modification")
                ...
                if cluster_id:
                    self.emr_client.modify_instance_groups(
                        ClusterId=str(cluster_id),
                        InstanceGroups=instance_group_configs
                    )
                else:
                    self.emr_client.modify_instance_groups(
                        InstanceGroups=instance_group_configs
                ...
                if cluster_id is None:
                    raise ValueError(
                        "cluster_id is required for list-instances operation"
                    )
                ...
                if instance_fleet_type is not None:
                    log_with_request_id(ctx, LogLevel.INFO, f"Filtering by instance fleet type: {instance_fleet_type}")
                if marker is not None:
                ...
                if instance_fleet_type is not None:
                    ...
                    log_with_request_id(ctx, LogLevel.INFO, f"Calling list_instances with fleet type: {instance_fleet_type}")
