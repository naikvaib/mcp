...
from awslabs.dataprocessing_mcp_server.handlers.glue.glue_commons_handler import GlueCommonsHandler
...
def handler(mock_mcp):
    with patch('awslabs.dataprocessing_mcp_server.handlers.glue.glue_commons_handler.AwsHelper') as mock_aws_helper:
...
class TestGlueCommonsHandler:
    ...
    @pytest.mark.asyncio
    async def test_manage_aws_glue_usage_profiles_create_no_write_access(self, mock_mcp, mock_context):
