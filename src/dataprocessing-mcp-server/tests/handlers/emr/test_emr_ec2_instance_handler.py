...
from awslabs.dataprocessing_mcp_server.handlers.emr.emr_ec2_instance_handler import EMREc2InstanceHandler
...
def emr_handler_with_write_access():
    ...
    mcp_mock = MagicMock()
    with patch('awslabs.dataprocessing_mcp_server.utils.aws_helper.AwsHelper.create_boto3_client') as mock_create_client:
...
class TestWriteOperationsPermissions:
    ...
    async def test_write_operations_denied_without_permission(
        ...
        assert any(f"Operation {operation} is not allowed without write access" in content.text 
...
class TestParameterValidation:
    ...
    async def test_invalid_operation_returns_error(self, emr_handler_with_write_access, mock_context):
    ...
    async def test_add_instance_fleet_parameter_validation(self, emr_handler_with_write_access, mock_context):
        ...
        with patch.object(emr_handler_with_write_access, 'emr_client'):
            with patch('awslabs.dataprocessing_mcp_server.utils.aws_helper.AwsHelper.prepare_resource_tags', return_value={}):
                # Mock to catch the ValueError instead of letting it propagate
                with patch.object(emr_handler_with_write_access, 'manage_aws_emr_ec2_instances', 
                                  side_effect=ValueError("cluster_id and instance_fleet are required for add-instance-fleet operation")):
    ...
    async def test_add_instance_groups_parameter_validation(self, emr_handler_with_write_access, mock_context):
        """Test that add-instance-groups validates required parameters."""
        with patch.object(emr_handler_with_write_access, 'emr_client'):
            with patch('awslabs.dataprocessing_mcp_server.utils.aws_helper.AwsHelper.prepare_resource_tags', return_value={}):
                with patch.object(emr_handler_with_write_access, 'manage_aws_emr_ec2_instances',
                                  side_effect=ValueError("cluster_id and instance_groups are required for add-instance-groups operation")):
                    with pytest.raises(ValueError) as excinfo:
                        await emr_handler_with_write_access.manage_aws_emr_ec2_instances(
                            ctx=mock_context,
                            operation="add-instance-groups",
                            instance_groups=[{"InstanceRole": "TASK", "InstanceType": "m5.xlarge", "InstanceCount": 2}]  # Missing cluster_id
                        )
                    assert "cluster_id" in str(excinfo.value)
    ...
    async def test_modify_instance_fleet_parameter_validation(self, emr_handler_with_write_access, mock_context):
        """Test that modify-instance-fleet validates required parameters."""
        with patch.object(emr_handler_with_write_access, 'emr_client'):
            with patch.object(emr_handler_with_write_access, 'manage_aws_emr_ec2_instances',
                              side_effect=ValueError("cluster_id, instance_fleet_id, and instance_fleet_config are required for modify-instance-fleet operation")):
                with pytest.raises(ValueError) as excinfo:
                    await emr_handler_with_write_access.manage_aws_emr_ec2_instances(
                        ctx=mock_context,
    ...
    async def test_modify_instance_groups_parameter_validation(self, emr_handler_with_write_access, mock_context):
        """Test that modify-instance-groups validates required parameters."""
        with patch.object(emr_handler_with_write_access, 'emr_client'):
            with patch.object(emr_handler_with_write_access, 'manage_aws_emr_ec2_instances',
                              side_effect=ValueError("instance_group_configs is required for modify-instance-groups operation")):
                with pytest.raises(ValueError) as excinfo:
                    await emr_handler_with_write_access.manage_aws_emr_ec2_instances(
                        ctx=mock_context,
    ...
    async def test_list_operations_parameter_validation(self, emr_handler_with_write_access, mock_context):
        """Test that list operations validate required parameters."""
        with patch.object(emr_handler_with_write_access, 'emr_client'):
            # Test list-instance-fleets
            with patch.object(emr_handler_with_write_access, 'manage_aws_emr_ec2_instances',
                              side_effect=ValueError("cluster_id is required for list-instance-fleets operation")):
                with pytest.raises(ValueError) as excinfo:
                    await emr_handler_with_write_access.manage_aws_emr_ec2_instances(
                        ctx=mock_context,
            ...
            with patch.object(emr_handler_with_write_access, 'manage_aws_emr_ec2_instances',
                              side_effect=ValueError("release_label is required for list-supported-instance-types operation")):
                with pytest.raises(ValueError) as excinfo:
                    await emr_handler_with_write_access.manage_aws_emr_ec2_instances(
                        ctx=mock_context,
...
class TestAddInstanceFleet:
    ...
    async def test_add_instance_fleet_success(self, emr_handler_with_write_access, mock_context):
        ...
        with patch.object(emr_handler_with_write_access, 'emr_client') as mock_emr_client:
            ...
            with patch('awslabs.dataprocessing_mcp_server.utils.aws_helper.AwsHelper.prepare_resource_tags') as mock_prepare_tags:
                ...
                assert any("Successfully added instance fleet" in content.text for content in result.content)
    ...
    async def test_add_instance_fleet_aws_error(self, emr_handler_with_write_access, mock_context):
        ...
        with patch.object(emr_handler_with_write_access, 'emr_client') as mock_emr_client:
            ...
            mock_emr_client.add_instance_fleet.side_effect = ClientError(
                error_response={"Error": {"Code": "ValidationException", "Message": "Invalid fleet configuration"}},
            ...
            assert result.isError is True
            assert any("Error in manage_aws_emr_ec2_instances" in content.text for content in result.content)
    ...
    async def test_add_instance_groups_success(self, emr_handler_with_write_access, mock_context):
        ...
        with patch.object(emr_handler_with_write_access, 'emr_client') as mock_emr_client:
            ...
            with patch('awslabs.dataprocessing_mcp_server.utils.aws_helper.AwsHelper.prepare_resource_tags') as mock_prepare_tags:
                ...
                assert any("Successfully added instance groups" in content.text for content in result.content)


class TestModifyInstanceFleet:
    """Test modify-instance-fleet operation."""
    
    async def test_modify_instance_fleet_with_valid_mcp_tags(self, emr_handler_with_write_access, mock_context):
        ...
        with patch.object(emr_handler_with_write_access, 'emr_client') as mock_emr_client:
            ...
            assert result.cluster_id == "j-12345ABCDEF"
            assert result.instance_fleet_id == "if-12345ABCDEF"
            assert any("Successfully modified instance fleet" in content.text for content in result.content)
    
    async def test_modify_instance_fleet_without_mcp_tags(self, emr_handler_with_write_access, mock_context):
    ...
    async def test_modify_instance_fleet_wrong_resource_type(self, emr_handler_with_write_access, mock_context):
...
class TestModifyInstanceGroups:
    ...
    async def test_modify_instance_groups_success(self, emr_handler_with_write_access, mock_context):
